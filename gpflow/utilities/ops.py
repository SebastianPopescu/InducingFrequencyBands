...
def batched_difference_matrix(X: tf.Tensor, X2: Optional[tf.Tensor]) -> tf.Tensor:
    ...
    if X2 is None:
        X2 = X
        #TODO -- need to think of a smarter way here
        #diff = X[..., :, tf.newaxis, :] - X2[..., tf.newaxis, :, :]
        #return diff
    Xshape = tf.shape(X)
    X2shape = tf.shape(X2)
    X = tf.reshape(X, (Xshape[0], -1, Xshape[-1]))
    
    X2 = tf.reshape(X2, (X2shape[0], -1, X2shape[-1]))
    
    diff = X[:, :, tf.newaxis, :] - X2[:, tf.newaxis, :, :]
    
    diff = tf.reshape(diff, tf.concat(([Xshape[0]], Xshape[1:-1], X2shape[1:-1], [Xshape[-1]]), 0))
