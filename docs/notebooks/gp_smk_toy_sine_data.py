...
plt.savefig('./figures/gp_multi_sinc_toy_data.png')
plt.close()
...
plt.savefig('./figures/gp_multi_sinc_toy_data_periodogram.png')
plt.close()
...
#kern = gpflow.kernels.MultipleSpectralBlock(n_components=N_COMPONENTS, means= means_np, 
#    bandwidths= bandwidths_np, powers=powers_np)
kern = gpflow.kernels.MixtureSpectralGaussian(n_components= N_COMPONENTS , powers = powers_np, means = means_np, bandwidths = bandwidths_np)

fig, ax = plt.subplots(1,1, figsize=(5, 2.5))

for _ in range(N_COMPONENTS):

    spectral_block_1a = make_component_spectrum(np.linspace(0, MAXFREQ, 1000), means_np[:,_], bandwidths_np[:,_], powers_np[_], use_blocks = False)
    ax.plot(np.linspace(0, MAXFREQ, 1000), spectral_block_1a.ravel(), label='$S_{aa}(\\nu)$', linewidth=.8)
